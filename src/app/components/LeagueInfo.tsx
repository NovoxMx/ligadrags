export default function LeagueInfo() {
    return (
      <section className="bg-[#1f1f1f] rounded-md shadow-lg p-6 mb-6">
        <h1 className="text-3xl text-[#33aaff] font-bold mb-2">Liga Drags 2025</h1>
        <p><strong>Juego:</strong> Rainbow Six Siege</p>
        <p><strong>Formato:</strong> Single Round Robin (Todos contra todos)</p>
        <p><strong>Fechas:</strong> 6 de Agosto 2025 - 10 de Agosto 2025</p>
        <p><strong>Premio Total:</strong> 400 MXN</p>

        <p className="mt-4">
          Esta liga fue creada con el propósito de divertirnos entre amigos y dejar registrado lo que pasó. Participan jugadores
          de ELO bajo, con el afán de diversión y para ver quién es el mejor jugador en 1 vs 1.
        </p>
      </section>
    );
  }